from preprocess import initial_image, OUTPUT_FOLDER_NAME, HUGGINGFACE_MODEL
...
# global variable
#HUGGINGFACE_MODEL = 'stablediffusionapi/juggernaut-xl-v8'
...
def generate_image(face_image, pose_image, prompt, negative_prompt, style_name, enhance_face_region, num_steps, identitynet_strength_ratio, adapter_strength_ratio, guidance_scale, seed, progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        print(f"HuggingFace model we will use right before submitting: {base_model_path}")
        ...
        print("Image generation successful on first try/catch.")
        ...
        try:
            ...
            # Take appropriate action (e.g., return an error message or a default image)
            return None, gr.update(visible=False), []

    # Return the result if no exceptions occurred or after handling them
    return images, gr.update(visible=True), generated_file_paths
...
try:
    ...
    logging.info("Launching demo interface...")
