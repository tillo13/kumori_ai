from preprocess import initial_image, OUTPUT_FOLDER_NAME
...
def generate_image(face_image, pose_image, prompt, negative_prompt, style_name, enhance_face_region, num_steps, identitynet_strength_ratio, adapter_strength_ratio, guidance_scale, seed, progress=gr.Progress(track_tqdm=True)):
    ...
    for idx, img in enumerate(images):
        ...
        filepath = os.path.join(output_dir_path, filename)  # Use the output directory path
        img.save(filepath)
        print(f"Image saved as {filepath}")
        
        generated_file_paths.append(filepath)  # Store the file path

    return images, gr.update(visible=True), generated_file_paths  # Return the file paths along with other return values
...
if device == 'mps' or device == 'cuda':
    ...
    initial_image(generate_image)  # Here we pass the generate_image function to preprocess_image
